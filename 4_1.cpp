...
int main(int argc, char **argv) {
  ...
  for (int bit_idx = 0; bit_idx < lines[0].size(); bit_idx++) {
    ...
    if (lines_to_consider_most_common.size() == 1 &&
        lines_to_consider_least_common.size() == 1)
      break;
    if (lines_to_consider_most_common.size() > 1) {
      int pos_count = 0;
      ...
      if (pos_count >= 0) {
        ...
        for (auto it = lines_to_consider_most_common.begin();
             it != lines_to_consider_most_common.end();) {
    ...
    if (lines_to_consider_least_common.size() > 1) {
      ...
      if (pos_count >= 0) {
        for (auto it = lines_to_consider_least_common.begin();
             it != lines_to_consider_least_common.end();) {
          const auto &line = lines[*it];
          if (line[bit_idx] == '1') {
